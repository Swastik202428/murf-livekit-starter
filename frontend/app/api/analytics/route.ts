...
try:
    table = conn.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='calls'").fetchone()
    if table is None:
        print(json.dumps({"total": 0, "successful": 0, "failed": 0, "active": 0, "success_rate": 0}))
    else:
...
`;
    ...
    console.error('Analytics dashboard SQLite query failed:', error);
